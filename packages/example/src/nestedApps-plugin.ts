...
import { RequestContextProvider, RPCServiceRegistry } from '@h4bff/backend';
...
export let NestedAppsPlugin = (app: App) => {
  // Root
  const ctxRouter = app.getSingleton(RequestContextProvider).router;
  ...
  child1.getSingleton(RequestContextProvider).router.get('/app/child1', (_req, res) => {
  ...
  child2.getSingleton(RequestContextProvider).router.get('/app/child2', (_req, res) => {
